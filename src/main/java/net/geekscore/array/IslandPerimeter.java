 ...
 * water. Grid cells are connected horizontally/vertically (not diagonally).
...
public class IslandPerimeter {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(perimeterOfIsland(grid));
    }
    
    private static int perimeterOfIsland(final int[][] grid){
        int perimeter = 0 ;
        if(grid == null || 0 == grid.length) return perimeter;
        for (int i = 0; i < grid.length; i++) {
            for (int j = 0; j < grid[i].length; j++) {
                if(grid[i][j] == 1) {
                    if(i == 0 || grid[i-1][j] == 0) perimeter++; // up
                    if(i == grid.length - 1 || grid[i+1][j] == 0) perimeter++; // down
                    if(j == grid[i].length -1 || grid[i][j+1] == 0) perimeter++; // right
